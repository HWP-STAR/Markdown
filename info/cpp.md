...
# 有关的cpp文件都要include .h文件
核心原因在于：.h 文件提供的是"声明"，而每个 .cpp 是独立编译的，编译器在每个编译单元里都需要看到这些声明。

# CPU 不区分“这是什么类型”，只认比特。
unsigned int：32 位无符号整数
float：IEEE 754 单精度浮点数，也是 32 位
...
S（1 位）：符号位
Exp（8 位）：指数（偏置 127）
Fraction（23 位）：尾数（小数部分）

对于偏置原因：
指数可以是 负的（比如 0.0001 = 2⁻¹³）
如果直接用一个 有符号 8 位整数：
范围：-128 ~ +127
比较大小、排序都麻烦
...
eg：
$$3.14 ≈ 11.00100011110101110000101  （2） $$
规格化：
$$1.100100011110101110000101 \times 2^1$$

32.56
0 10000100 00000100011110101110000
符号 指数 小数（二进制）
...
eg：pkg-config --modversion eigen3