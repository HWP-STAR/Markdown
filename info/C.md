...
# register int c; 就是给编译器一个"建议"：把这个变量放到寄存器里，这样访问更快。
int c; 是普通变量，放在内存（栈）里。**
除此之外，逻辑上完全一样。

# -> 是 C 语言里的结构体指针成员访问运算符。
一句话：
p->x 就是 (*p).x 的简写,访问成员（针对指针）

# math.h 使用
gcc编译 在 Linux / macOS 下，必须加 -lm（连接数学库）

因为 math.h 里的函数不在标准 C 库（libc）里，而是在一个独立的数学库（libm）里
